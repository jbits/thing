//
// Pimoroni Micro Dot Phat Enclosure
//
// by Yasuhiro Wabiko
//
...
module microdotphat_main(target="default"){
        ...
        if (with_body) {
            rotate(print_rotation[with_print]) {
                translate([stuff_x/2+holder_wall_t,0,0]){
                    difference() {
                        union() {
                            microdotphat_holder(with_round=1, with_hole_bottom=0, with_screw=0);
                            microdotphat_arm(with_welding=1);
                        }
                        union() {
                            microdotphat_holder_negative_volume(with_round=1, with_hole_bottom=0, with_screw=0);
                        }
                    }
                }
            }
    ...
    if (target == "screw_negative_test") {
        with_print = 1;
        rotate(print_rotation[with_print]) {
            microdotphat_screw(volume_type="negative");
        }
    }
    ...
    if (target == "unit_lid_negative_test") {
        with_print =0;
        rotate(print_rotation[with_print]) {
            microdotphat_unit_lid_negative_volume();
        }
    }
...
module microdotphat_unit() {
    translate([-holder_x, 0, holder_z/2]) {
        rotate([0,0,180]) {
            translate([holder_wall_t/2-holder_x/2,0,0]) {
                rotate([180,0,0]) {
                    microdotphat_holder(holder_y=36, holder_x=45, with_round=1, with_hole_front=0, with_hole_bottom=1, with_vent=0, with_screw=1);
                    microdotphat_arm(with_welding=1);
                }
            }
        }
...
module microdotphat_unit_lid_negative_volume() {
    translate([0, 0, holder_z/2]) {
        rotate([0,0,180]) {
            translate([holder_wall_t/2-holder_x/2-2,0,0]) {
                rotate([180,0,0]) {
                    microdotphat_holder(holder_y=36, holder_x=45, with_round=1, with_hole_front=0, with_hole_bottom=1, with_vent=0, with_screw=1);
                    microdotphat_arm(with_welding=1, with_board_part=1);
                    scale([1.1,1,1])
                    microdotphat_screw(volume_type="negative");
...
module microdotphat_arm(with_welding=1, left_cut_outs=[], right_cutouts=[],
        ...
        difference() {
            ...
            union() {
                ...
                translate(back_center(holder_x,0)) {
                    microdotphat_screw_hole_negative_volume();
                }
            }
        }
    }
...
module microdotphat_screw_hole_negative_volume() {
...
module microdotphat_holder(holder_x=holder_x, holder_y=holder_y,
    ...
    translate([-holder_wall_t/2,0,0]) {
        ...
        if (with_screw) {
            microdotphat_screw(volume_type="positive");
        }
    } // translate
}
...
// with_round = { 1: round the corners of holder's outer surface, 0: sharp (normal cube) edge. }
// with_round_inner = { 1: round the corners of holder's inner surface, 0: sharp (normal cube) edge. }
module microdotphat_holder_negative_volume(holder_x=holder_x, holder_y=holder_y,
                                           front_cutouts=holder_front_cutouts,
                                           bottom_cutouts=holder_bottom_cutouts,
                                           with_screw=0,
                                           with_hole_front=0,
                    ...
                    translate(top_center(holder_z, holder_wall_tz)) {
                        translate(front_center(board_x, arm_front_t+WELDING)) {
                            translate([4,0,0]) {
                                for (iy=[-7:1:7]) {
                                    translate([0, iy*4, 0]) {
                                        prim_round_zplate0(3, 2, holder_wall_tz*2, 1, 16);
                                    }
                                }
                            }
                        }
...
module microdotphat_screw(volume_type="positive") {
    holder_wall_ty = (holder_y - stuff_y)/2;
    holder_wall_tz = (holder_z - stuff_z)/2;
    holder_wall_tx = (holder_x - stuff_x)/2;
...
module microdotphat_lid(holder_x=holder_x, holder_y=holder_y,
                        front_cutouts=holder_front_cutouts,
                        bottom_cutouts=holder_bottom_cutouts,
                        with_screw=0,
                        with_hole_front=1,
